...
async  create(createGastosEmpresaDto: CreateGastosEmpresaDto): Promise<GastosEmpresa> {
...
if( createGastosEmpresaDto.idproyecto!=='' ){
      const foundProyecto: Proyecto = await this.proyectoRepository.findOne({
        where: { status: Not(StatusProyecto.CANCELADO),id: createGastosEmpresaDto.idproyecto },
      });
  if(!foundProyecto){
...
if(createGastosEmpresaDto.items.length<1){
  throw new BadRequestException("No hay una lista de productos en el gasto");
}
let valorTotal = 0;

for (let index = 0; index < createGastosEmpresaDto.items.length; index++) {
	if(parseFloat(createGastosEmpresaDto.items[index].cantidad.toString())>0 &&  parseFloat(createGastosEmpresaDto.items[index].importe.toString())>0 ){
 const gasto_item: GastoItem = new GastoItem();
 gasto_item.descripcion = createGastosEmpresaDto.items[index].descripcion;
 gasto_item.importe = createGastosEmpresaDto.items[index].importe;
 gasto_item.importeimpuesto = createGastosEmpresaDto.items[index].importeimpuesto;
 gasto_item.cantidad = createGastosEmpresaDto.items[index].cantidad;
 gasto_item.valortotal = parseFloat(gasto_item.importe.toString()) + parseFloat(gasto_item.importeimpuesto.toString());
...
const savedGasto: GastosEmpresa = await this.gastoRepository.save(creategasto);
if(!savedGasto){
throw new BadRequestException('Error al generar el gasto');
...
/*
 ...
 async remove(id: string ): Promise<GastosEmpresa> {
	 ...
	 const foundGasto: GastosEmpresa = await this.gastoRepository.findOne({where:{id: id, status:  Not(StatusGasto.CANCELADO) }});
	  if(!foundGasto){
		  
		  throw new NotFoundException("No existe el gasto o fue cancelado anteriormente");
	  }
